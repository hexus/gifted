package giftedclient {
	...
	public class maps extends MovieClip{
		...
		public static var solidarr:Array = new Array(3, 4, 5, 6 , 7 , 8, 9, 10);
		
		public var spawnX = 0;
		public var spawnY = 0;
		private var worldMap:Array;
		private var worldName:String;
		private var _outerMargin = 3;
		...
		private var tilePool:Vector.<tiles> = new Vector.<tiles>(); // Resource pool for tile objects
				...
				for(var j=0;j<wSize;j++){
					...
					for(var i=0;i<rSize;i++){
						...
						for(var h=0;h<rSize;h++){
							...
							newTile.val = 0;
							worldMap[k][j][i][h] = newTile; // Memory hog, sort it out
		...
		public function setMap(mapStr:String){ // So much simpler! Fucking love this algorithm
			var mapFlat = mapStr.split(",");
			var expectedSize = rSize*rSize*wSize*wSize;
			if(mapFlat.length == expectedSize){
				for(var i=0;i<mapFlat.length;i++){
					var rX = Math.floor(i / (rSize*rSize)) % wSize;
					var rY = Math.floor(i / (rSize*rSize*wSize)) % wSize;
					var cX = i % rSize;
					var cY = Math.floor(i / rSize) % rSize;
					setTile(rX,rY,cX,cY,mapFlat[i]);
				}
			}else{
				trace("Incorrectly sized world array ("+mapFlat.length+") received, expecting size "+expectedSize);
				return false;
			}
		}
		
		public function setRegion(getX:Number,getY:Number,gArr:Array):Boolean{
			if(gArr.length==regionSize*regionSize){
				...
				trace("Incorrectly sized region array received, try again");
				return false;
			}
		}
		...
		private function positionTile(rx:Number, ry:Number, cx:Number, cy:Number){
			...
			if(newTile!=null){
				newTile.x = ((rx*regionSize) + cx) * _tileW;
				newTile.y = ((ry*regionSize) + cy) * _tileH;
			}
		}
		...
		private function addTile(cx:Number,cy:Number){
			...
			newtile.x = cx * _tileW; // Half add because tiles
			newtile.y = cy * _tileH; // are center oriented
			if(!newtile.clip){
				newtile.clip = new tiles();
			}
			addChild(newtile.clip);
		}
		
		private function removeTile(t){
			if(t is tiles){
				t = t.data;
			}
			if(t.clip){
		...
		public function clearDisplay(){
			// Remove all children
				while(numChildren>0){
					var c = removeChild(getChildAt(0));
					c = null;
				}
		}
		
		public function updateDisplay(dx=null,dy=null,scale=1,t=1,full=false):void{
			//trace(numChildren + " : " + tilePool.length);
			if(!dx && lastUpdated[0]){dx = lastUpdated[0];}
			if(!dy && lastUpdated[1]){dy = lastUpdated[1];}
			if(scale!=lastScale){clearDisplay();full=true;}
			if(t<1){t=1;}
			...
			// Remove current children outside view bounds (redundant?)
			...
			}else{ // Iterated over edges of viewport to add and remove
				if(dX1!=lastUpdated[2]){
					var dx1rel = dX1 - lastUpdated[2];
					for(var k=0; k < Math.abs(dx1rel); k++){
						for(var l=((dY1<lastUpdated[4])?dY1:lastUpdated[4]); l<((dY2>lastUpdated[5])?dY2:lastUpdated[5]); l++){ // This needs to cater for previous Y vals as well
							var addX,remX;
							if(dx1rel>0){ // Moving left
								remX = lastUpdated[2]+k;
								addX = lastUpdated[3]+k;
							}else{ // Right
								remX = lastUpdated[3]-k;
								addX = lastUpdated[2]-k;
							}
							cord = convertCords(remX,l,true); rx = cord["rx"]; ry = cord["ry"]; cx = cord["x"]; cy = cord["y"];
							var tileRemove = worldMap[ry][rx][cy][cx];
							removeTile(tileRemove);
							if(l>=dY1 && l<=dY2){
								addTile(addX,l);
							}
						}
				...
				if(dY1!=lastUpdated[4]){
					var dy1rel = dY1 - lastUpdated[4];
					for(var m=0; m < Math.abs(dy1rel); m++){
						for(var n=((dX1<lastUpdated[2])?dX1:lastUpdated[2]); n<((dX2>lastUpdated[3])?dX2:lastUpdated[3]); n++){
							var addY,remY;
							if(dy1rel>0){
								remY = lastUpdated[4]+m;
								addY = lastUpdated[5]+m;
							}else{
								remY = lastUpdated[5]-m;
								addY = lastUpdated[4]-m;
							}
							cord = convertCords(n,remY,true); rx = cord["rx"]; ry = cord["ry"]; cx = cord["x"]; cy = cord["y"];
							tileRemove = worldMap[ry][rx][cy][cx];
							removeTile(tileRemove);
							if(n>=dX1 && n<=dX2){
								addTile(n,addY);
							}
			...
			lastUpdated = [dx,dy,dX1,dX2,dY1,dY2]; // Remember last updated coords
			lastScale = scale;
